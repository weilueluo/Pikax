...
class IllustrationScreen(PikaxGuiComponent):
    ...
    def __init__(self, master, pikax_handler):
        ...
        self.grid_width = 20
        self.grid_height = 18
        self.id_or_url_text_id = self.add_text(text=texts.ILLUSTRATION_ID_OR_URL, row=4, column=9, columnspan=2)
        ...
        # buttons
        self.download_button = self.make_button(text=texts.ILLUSTRATION_DOWNLOAD)
        self.download_button_id = self.add_widget(widget=self.download_button, row=13, column=11)
        self.back_button = self.make_button(text=texts.ILLUSTRATION_BACK)
    ...
    def download_clicked(self):
        ...
        if search_ids:
            ...
            sys.stdout.write(texts.ILLUSTRATION_NO_ID_FOUND)

    def make_switchbutton(self, values, default=None, *args, **kwargs):
        button = super().make_switchbutton(values, default, *args, **kwargs)
        button.configure(font=self.text_font)
        return button
